...
const ViewStockO=()=>{
  ...
  const getdata=async(e)=>{
    ...
    const res =await fetch("http://localhost:8070/stockorder",{
...
const deleteuser = async (id) => {
    ...
    const res2 = await fetch(`http://localhost:8070/stockorder/delete/${id}`, {
        method: "DELETE",
        headers: {
 ...
 <div className="table-wrap">
  ...
  <tbody>
    ...
    {getuserdata.filter((val)=>{
      ...
      <NavLink to={`/updateStock/${element._id}`}><button className="btn btn-dark" >Update</button></NavLink>
